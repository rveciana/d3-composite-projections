...
import geoAlbersUsaTerritories from "../src/albersUsaTerritories.js";

tape("geoAlbersUsa(point) returns the expected result", function (test) {
  var albersUsa = geoAlbersUsaTerritories(); /*
...
tape(
  "geoAlbersUsaTerritories added zones return the expected results",
  function (test) {
    var proj = geoAlbersUsaTerritories();

    var locations = [
      { name: "San Juan", coords: [-66.1, 18.43] },
...
tape(
  "geoAlbersUsaTerritories.getCompositionBorders() returns the expected result",
  function (test) {
    var albersUsa = geoAlbersUsaTerritories();
    ...
    test.equal(
      ...
      20,
      "Number of border lines must be 20"
    );
    test.equal(
      (borders.match(/M/g) || []).length,
      5,
      "Number of borders must be 5"
    );
    test.end();
  }
);
