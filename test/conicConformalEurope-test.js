...
tape(
  "geoConicConformalEurope(point) returns the expected result",
  function (test) {
    var proj = geoConicConformalEurope();
...
tape(
  "geoConicConformalEurope.getCompositionBorders() returns the expected result",
  function (test) {
    var borders = geoConicConformalEurope().getCompositionBorders();
    test.equal(
      (borders.match(/L/g) || []).length,
      33,
      "Number of border lines must be 33"
    );
    test.equal(
