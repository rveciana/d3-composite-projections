...
tape(
  "geoConicConformalFrance(point) returns the expected result",
  function (test) {
    var proj = geoConicConformalFrance();
    ...
    var locations = [
      ...
      { name: "Gustavia", coords: [-62.85194, 17.89583] },
    ];
...
tape(
  "geoConicConformalFrance.getCompositionBorders() returns the expected result",
  function (test) {
    var borders = geoConicConformalFrance().getCompositionBorders();
    test.equal(
      (borders.match(/L/g) || []).length,
      31,
      "Number of border lines must be 31"
    );
    test.equal(
      (borders.match(/M/g) || []).length,
      11,
      "Number of borders must be 11"
    );
    test.end();
  }
);
