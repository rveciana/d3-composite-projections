...
tape(
  "geoTransverseMercatorChile(point) returns the expected result",
  function (test) {
    var proj = geoTransverseMercatorChile();
    var locations = [
      { name: "Santiago", coords: [-70.6, -33.5] },
...
tape(
  ...
  function (test) {
    var borders = geoTransverseMercatorChile().getCompositionBorders();
    test.equal(
      (borders.match(/L/g) || []).length,
      12,
      "Number of border lines must be 12"
    );
    test.equal(
